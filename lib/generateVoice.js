...
// Gemini TTS는 raw PCM(16-bit, mono, 24kHz)만 반환하고 컨테이너가 없어서, 재생/후속 처리가
// 가능하도록 표준 WAV 헤더를 직접 씌운다.
...
async function synthesizeWithGemini({ text, voice, credentials, speed }) {
  const apiKey = credentials?.apiKey || process.env.GEMINI_API_KEY;
  if (!apiKey) {
    throw new Error('GEMINI_API_KEY 환경변수가 필요합니다.');
  }

  // 실측(2026-09-14): 입력 text + 프롬프트 합쳐서 8,000바이트 제한 — 한글은 글자당 약 3바이트라
  // 대략 2,500자가 실질 상한이다. 이 함수는 청킹을 하지 않으므로, 호출하는 쪽에서 8,000바이트를
  // 넘지 않는 단위(챕터/씬 등)로 text를 나눠서 넘겨야 한다. 넘으면 Gemini가 조용히 잘라서
  // 일부만 반환한다(에러 없음) — 결과 오디오 길이가 예상보다 짧으면 이 제한을 의심할 것.
  ...
  if (!res.ok) {
    const errText = await res.text().catch(() => '');
    throw new Error(`Gemini TTS 요청 실패 (${res.status}): ${errText.slice(0, 300)}`);
  }
  ...
  if (!base64Audio) {
    throw new Error(`Gemini TTS 응답에 오디오가 없습니다: ${JSON.stringify(json).slice(0, 300)}`);
  }

  const pcmBuffer = Buffer.from(base64Audio, 'base64');
  const audioBuffer = pcmToWav(pcmBuffer);
  // Gemini TTS는 문자 단위 타임스탬프를 제공하지 않는다 → buildCaptions.js가 균등분배로 근사한다.
  // speed 파라미터는 API에 없음 — 필요하면 buildCaptions/렌더링 단계에서 배속 처리할 것.
  return { audioBuffer, alignment: null };
}
 ...
 * @param {'fal'|'elevenlabs'|'clova'|'gemini'} [params.provider] - 지정 안 하면 TTS_PROVIDER 환경변수(기본 fal) 사용
...
export async function synthesizeVoice({ text, voice, provider, credentials, speed }) {
  ...
  if (resolvedProvider === 'gemini') {
    return synthesizeWithGemini({ text, voice, credentials, speed });
  }
