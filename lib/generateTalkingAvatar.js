...
export async function generateTalkingAvatar({ imageUrl, text, voice = 'Aria', prompt, resolution = '480p' }) {
  ...
  for (let i = 0; i < MAX_POLL_ATTEMPTS; i++) {
    const statusRes = await fetch(`https://queue.fal.run/${MODEL_ID}/requests/${requestId}/status`, {
  ...
  const resultRes = await fetch(`https://queue.fal.run/${MODEL_ID}/requests/${requestId}`, {
    headers: { Authorization: `Key ${apiKey}` },
  });
