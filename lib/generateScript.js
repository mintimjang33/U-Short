...
const STYLE_GUIDE = {
  ...
  shopping:
    '쇼핑 구매유도형: 입력에 포함된 상품명과 가격을 자연스럽게 언급하고, 이 상품의 장점·활용도를 짧고 강렬하게 짚어준 뒤 "지금 확인해보세요" 같은 구매 유도 멘트로 마무리한다. 근거 없는 효능 단정("100% 효과", "완치")이나 과장 광고성 문구는 쓰지 않는다.',
  // "신비한 건축사전" 등 지식형 채널 벤치마킹(2026-08) 분석 결과 반영: 다들 알고 있다고 믿는
  // 상식을 첫 문장에서 뒤집는 "예측 오류" 구조 — 문제 제기 → 반전(사실은 ~다) → 과학적/역사적
  // 근거 설명 → 임팩트 있는 마무리 문장 순서. AI 영상 생성 모드(planScenes.js)와 특히 잘 맞는다.
  'twist-reveal':
    '반전 지식형: 첫 문장에서 널리 알려진 상식을 제시한 뒤 "사실은 그게 아니다"라며 곧바로 뒤집는 반전으로 시작한다. 이어서 그 반전의 진짜 원인/배경을 구체적 수치나 사건과 함께 설명하고, 마지막은 여운이 남는 한 문장으로 마무리한다. 소재를 그대로 베끼지 말고 사실관계만 참고해서 완전히 새로운 문장으로 쓴다.',
};
...
function buildSystemPrompt({ style, customStyleDescription, outputLanguage, planningMode, lengthMode, targetChars }) {
  // customStyleDescription(레퍼런스 대본 학습으로 저장해둔 스타일)이 있으면 STYLE_GUIDE 프리셋보다 우선한다.
  const styleDesc = customStyleDescription || STYLE_GUIDE[style] || STYLE_GUIDE.summary;
  ...
  // targetChars(자유 입력)가 있으면 lengthMode(3단계 프리셋)보다 우선한다 — 정확한 글자수 목표가
  // 필요할 때(예: 특정 초 길이에 정확히 맞춰야 하는 경우) 쓰라고 추가함.
  let charMin, charMax, secMinLabel, secMaxLabel;
  if (targetChars) {
    charMin = Math.max(30, Math.round(targetChars * 0.85));
    charMax = Math.round(targetChars * 1.15);
    secMinLabel = Math.round(charMin / APPROX_CHARS_PER_SECOND);
    secMaxLabel = Math.round(charMax / APPROX_CHARS_PER_SECOND);
  } else {
    const lengthConfig = LENGTH_MODES.find((l) => l.id === lengthMode) || LENGTH_MODES[0];
    charMin = Math.round(lengthConfig.secMin * APPROX_CHARS_PER_SECOND);
    charMax = Math.round(lengthConfig.secMax * APPROX_CHARS_PER_SECOND);
    secMinLabel = lengthConfig.secMin;
    secMaxLabel = lengthConfig.secMax;
  }
  const lengthInstruction =
    planningMode === 'direct'
      ? null // direct 모드는 사용자가 이미 정한 분량을 그대로 쓰므로 글자수를 강제하지 않는다.
      : `내레이션 분량은 낭독 기준 ${secMinLabel}~${secMaxLabel}초 분량인 ${charMin}~${charMax}자 내외로 만든다. 짧게 쓰지 말고 이 범위를 채울 것.`;
 ...
 * @param {'shortform'|'longform'|'extended'} [params.lengthMode] - 목표 낭독 길이, 기본 shortform(10~20초). targetChars가 있으면 무시됨.
 * @param {number} [params.targetChars] - 목표 글자수 자유 입력(30자 이상). 지정하면 lengthMode 대신 이 값 기준으로 분량을 맞춘다.
 * @param {string} [params.customStyleDescription] - 레퍼런스 대본 학습으로 저장해둔 커스텀 스타일 지침(analyzeScriptStyle.js 결과). 지정하면 style 프리셋 대신 이걸 쓴다.
...
export async function generateScript({
  ...
  if (targetChars !== undefined && (typeof targetChars !== 'number' || targetChars < 30)) {
    throw new Error('generateScript: targetChars는 30 이상의 숫자여야 합니다.');
  }

  const system = buildSystemPrompt({ style, customStyleDescription, outputLanguage, planningMode, lengthMode, targetChars });
