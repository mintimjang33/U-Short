 ...
 *
 * provider: 'google-flow'면 fal API 대신 사용자의 Google Flow 구독 크레딧을 크롬 확장
 * 프로그램으로 대행 호출한다(lib/generateImageViaFlow.js, chrome-extension-flow/ 참고) —
 * 종량과금 없이 더 저렴하지만 확장 프로그램이 켜져 있어야 하고 계정 리스크가 있다.
 */
import { generateImageViaFlow } from './generateImageViaFlow.js';
 ...
 * @param {'nano-banana'|'google-flow'} [params.provider] - 기본 nano-banana(fal 정식 API)
 * @returns {Promise<{imageUrl: string, width?: number, height?: number}>}
 */
export async function generateImage({ prompt, referenceImageUrls, aspectRatio = '9:16', provider = 'nano-banana' }) {
  if (provider === 'google-flow') {
    return generateImageViaFlow({ prompt, aspectRatio, referenceImageUrls }, { supabase: getSupabaseServerClient() });
  }
