...
export async function runCutDaeriRender({ projectId }) {
  ...
  for (const cut of cuts) {
    if (!(cut.image_url || cut.video_url) || !cut.audio_url) {
      throw new Error(`컷 ${cut.order_index + 1}에 이미지/동영상 또는 음성이 없습니다. U-OneShot에서 먼저 전부 생성해주세요.`);
    }
    ...
    // video_url이 있으면(컷비서 3단계 "동영상" 옵션) 정지 이미지 대신 그 클립을 쓴다.
    scenes.push({ imageUrl: cut.image_url, videoUrl: cut.video_url || null, audioUrl: cut.audio_url });
    cursorMs += durationMs;
  }
