...
export async function runInstatoonPipeline({ projectId }) {
  ...
  try {
    ...
    for (let i = 0; i < plannedPanels.length; i++) {
      ...
      const fullPrompt = panel.text
        ? `${panel.imageDescription}, the image includes bold Korean caption text "${panel.text}" as a speech bubble or caption overlay`
        : panel.imageDescription;

      const { imageUrl: falUrl } = await generateImage({ prompt: fullPrompt, referenceImageUrls, aspectRatio: '1:1' });
      const imgRes = await fetch(falUrl);
      if (!imgRes.ok) throw new Error(`컷 ${i + 1} 이미지 다운로드 실패 (${imgRes.status})`);
      const buffer = Buffer.from(await imgRes.arrayBuffer());
      const publicUrl = await uploadImage(supabase, `instatoon/${projectId}/panel-${i + 1}.png`, buffer);
