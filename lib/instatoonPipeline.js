  ...
  try {
    ...
    if (project.character_style_set_id) {
      const { data: set } = await supabase
        ...
        .select('reference_image_urls, learned_rules')
        .eq('id', project.character_style_set_id)
        .maybeSingle();
      referenceImageUrls = set?.reference_image_urls || [];
      learnedRules = set?.learned_rules || '';
    }
    ...
    for (let i = 0; i < plannedPanels.length; i++) {
      ...
      const panelPrompt = learnedRules
        ? `${panel.imageDescription}\n\nStyle rules learned from past corrections:\n${learnedRules}`
        : panel.imageDescription;
      const { imageUrl: falUrl } = await generateImage({ prompt: panelPrompt, referenceImageUrls, aspectRatio: '1:1' });
