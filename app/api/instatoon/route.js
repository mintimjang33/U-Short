...
export const GET = withApiErrorHandling(async () => {
  ...
  let query = supabase.from('instatoon_projects').select('*').order('created_at', { ascending: false });
...
// (create_shorts/app/api/jobs와 동일한 큐잉 패턴, Vercel 서버리스 시간제한 회피 목적).
export const POST = withApiErrorHandling(async (request) => {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      topic: body.topic,
      panel_count: body.panelCount || 6,
      character_style_set_id: body.characterStyleSetId || null,
      status: 'queued',
    })
    .select()
    .single();
