...
5. **관리자 전용 워커 패키지 다운로드**: API/UI 코드는 배포됐으나(`app/api/admin/worker-package/route.js`), 실제 zip 파일(`ushort-worker-package.zip`)을 Supabase Storage `releases` 버킷에 아직 안 올림 — 사용자가 수동으로 업로드하기로 함(자동화 스크립트가 분류기에 차단됨). zip은 소스코드만 있고 `.env.local`은 빠져있음, 사용자가 직접 추가할지 결정 필요.
