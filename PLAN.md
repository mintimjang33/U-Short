...
1. **유튜브 자막 지원**: `playwright-core` + 이 PC의 Chromium(`chrome-headless-shell.exe`)으로 실제 자막 패널 긁기를 시도했으나 **재현 불가능하게 불안정함 확인**(한 번은 페이지 로드+버튼 클릭까지 성공, 이후 시도들은 매번 다른 지점—`goto`, `close()`, 버튼 카운트—에서 멈춤). 원인 후보: 이 PC의 `chrome.exe`가 손상됨("side-by-side configuration 오류") + 디스크 거의 꽉 참(4.48GB 남음) + `chrome-headless-shell.exe`가 YouTube 같은 무거운 SPA엔 불안정할 수 있음. **다음 세션에서 이어갈 것**: 디스크 공간 확보 후 `npx playwright install chromium`으로 깨끗한 바이너리 새로 받아서 재시도, 또는 제목+설명만 쓰는 낮은 품질 폴백으로 우선 진행할지 결정 필요.
...
5. ~~**관리자 전용 워커 패키지 다운로드**~~ → **완료**: 사용자가 `ushort-worker-package.zip`을 Supabase Storage `releases` 버킷에 직접 업로드 완료함(2026-08-21, 스크린샷으로 확인). `/admin` 페이지의 "워커 패키지 다운로드" 버튼으로 관리자 로그인 시에만 다운로드 가능. zip은 소스코드만 있고 `.env.local`은 빠져있음 — 원하면 사용자가 직접 zip에 추가해서 재업로드 가능(선택사항, 안 해도 새 PC에서 수동으로 채우면 됨).

**→ 보류 항목(ElevenLabs, Voicebox) 제외하면 실질적으로 "유튜브 자막 지원"만 남은 상태(2026-08-21 기준).**
