...
## Voicebox 로컬 음성 클로닝 시도 → 보류 (2026-08-20)

- Voicebox(오픈소스, MIT, voicebox.sh) 설치 완료(`Voicebox_0.5.0_x64_en-US.msi`), REST API(`http://127.0.0.1:17493`) 정상 확인.
- **Qwen TTS 0.6B**: 로드 시도했으나 Voicebox 자체의 알려진 버그로 실패(GitHub 오픈 이슈 #878, "Unrecognized model... qwen3_tts" 에러, 미해결 상태).
- **Chatterbox**(23개 언어, 한국어 포함, Resemble AI): 다운로드 시도 중 이 PC 디스크(118GB 중 거의 다 참)가 완전히 꽉 차서 실패. 실제 전체 용량은 13.87GB로 예상보다 훨씬 컸음(사전에 API로 정확한 크기 확인 안 하고 시작한 게 원인). 미완성 다운로드분(2.64GB)은 삭제해서 공간 일부 복구함.
- **LuxTTS**(1.18GB, 지금 여유공간으로 가능한 용량)는 한국어 지원이 불확실(HF 페이지에 "English"만 명시) — 확인 안 하고 받는 건 보류.
- 대안으로 "다른 PC를 Voicebox 원격 서버로 쓰기", "미니PC 구매(워커 상시서버 + 선택적 GPU)" 논의했으나, **사용자가 최종적으로 "우선 fal로 하고 워커[미니PC]는 필요 없다"고 결정** — Voicebox/로컬 클로닝은 보류, fal.ai 페르소나 23종(이미 구현/배포됨)으로 계속 진행하기로 함.
- 이 PC에 Voicebox 앱 자체는 설치된 채로 남아있음(모델 없이는 그냥 대기 상태, 문제 없음). 나중에 다시 시도하고 싶으면 디스크 공간 확보 + Chatterbox(한국어 확실, 13.87GB) 다운로드부터 이어가면 됨.

### 나중에 전용 서버(미니PC) 마련하면 이어서 할 것

- **워커 상시서버용**(GPU 불필요, Remotion 렌더링만): Intel **N100/N150** 기반 저전력 미니PC 추천 확인함 — **Beelink EQ12**(N100, 초보자 추천), **GMKtec** N150 모델(밸런스 좋음). 스펙 기준: RAM 16GB 권장(8GB는 헤드리스 브라우저 렌더링에 빠듯함), SSD 512GB 이상(이 노트북이 118GB로 디스크 꽉 차서 고생했음), 가격대 20~40만원.
- **로컬 음성 클로닝(Voicebox/Chatterbox)까지 하려면**: 일반 미니PC(내장그래픽)로는 안 됨 — 진짜 CUDA GPU 있는 "미니 게이밍PC"(RTX 탑재) 카테고리라 훨씬 비쌈. fal.ai 비용이 영상 한 편에 10~50원 수준이라 순수 비용 대비로는 GPU 장비 구매 실익이 낮음 — 상시서버 겸 쓸 목적이 아니면 굳이 안 사도 됨.
- **원격 서버 구조**(대안, 구매 없이): 다른 PC(디스크 여유 있는 곳)에 Voicebox 설치해서 `http://<그PC IP>:17493`으로 이 PC(워커)에서 원격 호출하는 방식도 가능 — 같은 네트워크에 있으면 추가 설정 거의 불필요.
- **결정**: 2026-08-20 기준 사용자가 "우선 fal로 하고 워커[미니PC]는 필요 없다"고 확정 — 서버/미니PC 구매는 보류, 나중에 필요해지면 위 내용 참고해서 재개.

## 다음에 할 일 (2026-08-21 기준, 사용자가 우선순위 재확정함)

1. ~~**유튜브 자막 지원**~~ → **사용자가 "우선 보류"로 명시함(2026-08-21)**. `playwright-core` + 이 PC의 Chromium(`chrome-headless-shell.exe`)으로 실제 자막 패널 긁기를 시도했으나 **재현 불가능하게 불안정함 확인**(한 번은 페이지 로드+버튼 클릭까지 성공, 이후 시도들은 매번 다른 지점—`goto`, `close()`, 버튼 카운트—에서 멈춤). 원인 후보: 이 PC의 `chrome.exe`가 손상됨("side-by-side configuration 오류"). **재개 시 첫 단계**: `npx playwright install chromium`으로 깨끗한 바이너리 새로 받아서 재시도(단, 이건 원인 후보 하나를 제거하는 것뿐이라 성공을 보장하진 않음 — 유튜브의 PO 토큰 방어 자체가 자동화하기 까다로운 영역).
2. ~~**ElevenLabs 무료 티어 음성 매핑**~~ → **사용자가 "추후"로 명시적으로 보류함(2026-08-21)**. 재개 조건: 사용자가 elevenlabs.io API 키 발급해서 전달.
3. ~~**Voicebox 로컬 음성 클로닝**~~ → **사용자가 "추후"로 명시적으로 보류함(2026-08-21)**. 위 "Voicebox 로컬 음성 클로닝 시도 → 보류" 섹션 참고.
4. ~~`npm run worker`를 이 PC에서 상시 실행 상태로 유지~~ → **완료**: 바탕화면 "UShort 워커 시작" 바로가기(`워커_시작.bat`) 생성함.
5. ~~**관리자 전용 워커 패키지 다운로드**~~ → **완료(초기버전)**: 사용자가 `ushort-worker-package.zip`을 Supabase Storage `releases` 버킷에 직접 업로드 완료함(2026-08-21). 이후 계정/폴더/TTS키 등 기능이 대거 추가돼서 zip이 최신 소스와 안 맞음 — **사용자가 "마지막에 추가"하라고 명시함(2026-08-21)**: 다른 작업 다 끝난 뒤 최신 소스로 zip을 다시 만들어서 재업로드할 것.

**→ 보류 항목(ElevenLabs, Voicebox, 유튜브 자막, 결제/사용량/제휴) 제외하면, 워커 패키지 최신화(맨 마지막 예정)만 남은 상태(2026-08-21 기준).**

## 실사이트 전체 재감사 결과 (2026-08-21, "제대로 클론해오라"는 피드백 이후)

사용자가 "클론 똑바로 해오라고 한 것도 찾아보면 자꾸 없는 페이지들이 있다"고 격하게 지적 → 공지사항 로그 + 사이드바 + `/account` + `/revenue-share/dashboard`까지 실사이트를 처음부터 다시 systematic하게 훑어서 격차를 전부 찾아냄.

**완료된 것 (9개 기능 + 44종 템플릿 + 계정/폴더):**
1~9. 스톡영상 자동추천, QR 핸드폰 업로드, 인터넷조사 대본, 상세편집, 이미지슬롯, 로고/영상 오버레이, 자막 애니메이션(팝/흔들기/불타는자막), 내 목소리 녹음, 장면별 자막프리셋 — 전부 완료·배포됨.
- **44종 템플릿의 정체**: 레이아웃 5종 × 제목(title) 색상/강조 스타일 8종의 조합이었음(사이트에 "예시 썸네일, 스타일만 적용" 명시됨). `remotion/src/titlePresets.js` + `TitleBlock.jsx` 공통 컴포넌트로 5개 레이아웃 전부에 적용 완료.
- **`/account` 내 정보 페이지**: 이름 수정, 계정 삭제(탈퇴) 완료. 사이드바에 이메일 클릭하면 진입.
- **개인 TTS API 키 등록**: `/account` API 키 탭에서 ElevenLabs/CLOVA 개인 키 등록 가능, `user_api_keys` 테이블에 저장, `pipeline.js`가 전역 키보다 우선해서 사용하도록 연결 완료.
- **프로젝트 폴더 + 검색**: `/dashboard`에 폴더 생성/이동, 검색창(클라이언트 필터) 추가 완료. `folders` 테이블 + `projects.folder_id` 컬럼 필요(마이그레이션 SQL 별도 전달함).

**사용자가 "추후"로 명시적으로 보류(2026-08-21):**
- **결제 관리**(카드 등록/구독/청구서) — 실제 결제 게이트웨이 연동 필요, 위험도 높음
- **사용량/크레딧 제한 시스템** — 결제와 연동되는 개념이라 같이 보류
- **제휴 파트너/수익공유 프로그램**(추천코드, 10% 커미션, 계좌 등록, 월별 정산) — 다수 유료회원 대상 비즈니스 기능, 계좌정보 등 민감정보 다룸
...
### 인스타툰 (N컷 카드형 웹툰) 구현 + 한글 텍스트 버그 발견·수정 (2026-08-30, 커밋 5bde593+b61a2c0, 실사용 검증 완료)

⑤⑥ 위에 구축한 새 프로젝트 타입. `lib/planInstatoon.js`(주제→N컷 기획, 컷마다 캡션+imageDescription),
`lib/instatoonPipeline.js`(기획→컷별 이미지 생성, DB 진행상황 갱신), 새 테이블 `instatoon_projects`
(jobs/uos_cutdaeri_projects와 동일한 큐잉 컨벤션 — `scripts/worker.js`에 3번째 폴링 대상으로 추가),
MCP 도구 `create_instatoon`/`get_instatoon_status`, 새 페이지 `/instatoon`.

**1차 구현 후 실제 테스트에서 발견한 버그**: fal Nano Banana 프롬프트에 한글 캡션을 직접 포함시켰더니
글자가 그림처럼 그려지다 다른 글자로 깨짐("오늘도"→"을들도", "벌써"→"별조", "지각이야"→"지작아야" 등)
— AI 이미지 모델의 고질적인 비라틴 문자 렌더링 한계. 캐릭터 일관성 자체는 이 단계에서도 완벽했음
(같은 여성이 3개 장면에서 얼굴형·헤어·화풍 전부 일관되게 유지됨, 스크린샷으로 확인).

**수정**(커밋 b61a2c0): 텍스트를 프롬프트에 굽지 않고, `remotion/src/layouts/InstatoonPanel.jsx`(신규,
1080x1080 정지 프레임 컴포지션) + `lib/render.js`의 `renderInstatoonPanel()`(renderMedia 대신
가벼운 `renderStill` 사용)로 배경이미지+말풍선 텍스트를 별도 합성 — 실제 폰트 렌더링이라 100% 정확.
캐릭터 일관성용 자동 레퍼런스도 텍스트 합성 전 순수 배경 이미지(`-bg.png`)를 쓰도록 같이 수정
(말풍선까지 캐릭터로 착각해 다음 컷에 따라 그리는 것 방지).

**최종 검증**: 로컬 워커를 실제로 켜서(`.env.local` 새로 세팅 — `NEXT_PUBLIC_SUPABASE_URL`/
`SUPABASE_SERVICE_ROLE_KEY`는 사용자가 Supabase 대시보드에서 직접 가져다줌) 실제 라이브 MCP로
"강아지와 산책하는 즐거운 오후" 요청 → AI가 "책 안 읽는 습관"으로 재해석해 3컷 생성 → 워커가 실제로
처리 완료 → **3컷 전부 한글 100% 정확**("오늘도 책 안 읽었지?" / "내일부터 하루 10페이지!" /
"...내일도 또 내일"), 같은 사람·같은 거실이 3컷 내내 완벽하게 일관됨. 다운로드해서 스크린샷으로
직접 확인 완료. 새 테이블은 `supabase/migration_instatoon_projects.sql`로 전달, 사용자가 실행함.

**한글 렌더링 버그 처리 중 겪은 삽질**: 첫 단독 테스트 때 `renderStill` 출력 경로를 Downloads 루트의
한글 폴더명("유쇼츠") 하위로 잡았더니 성공 메시지가 떴는데도 실제로는 파일이 안 만들어지는 문제가
있었음(Remotion/Chromium의 비ASCII 경로 처리 이슈로 추정) — `os.tmpdir()`(비한글 경로)로 테스트하니
정상 동작 확인. 실제 파이프라인 코드는 처음부터 `os.tmpdir()`을 쓰므로 프로덕션에는 영향 없음.
